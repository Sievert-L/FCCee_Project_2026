...
output_dir_name = f'OUTPUT_performance_scans/Performance_scans_ideal_lattice'
...
study_param['ini_cond_nemittance_y'] = 0.7e-12 * 89236 # 0.7pm when no collisions otherwise 1.4pm
study_param['ini_cond_type'] = 'grid_MA'
study_param['ini_cond_energy_spread'] = 0.39e-3 # 0.39e-3 when no collisions otherwise 1.34e-3
study_param['ini_cond_bunch_length'] = 5.1e-3 # 5.1mm when no collisions otherwise 16.7mm


# -------------------------------- Ideal line -------------------------------- #
line = xt.Line.from_json(f'{path}/12_LCC_V106/line_fccee_p_ring_{line_version}_merged_dipoles_with_correctors.json') # reference line
...
if study_param['ini_cond_type'] == 'grid_MA':
    delta_initial_values = np.linspace(-0.020, 0.020, 100) # 100 is the number of points along x # changed from 100 to 120
