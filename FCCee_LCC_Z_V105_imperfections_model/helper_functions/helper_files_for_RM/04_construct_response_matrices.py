...
OBSERVATION_POINTS = json.load(open('observation_points.json', 'r'))
# OBSERVABLES = ['mux', 'muy', 'dx'] # for normal correctors
OBSERVABLES = ['f1001s', 'f1001c', 'f1010s', 'f1010c', 'dy'] # for skew correctors
...
# Uncomment this for the normal correctors
# # Build response matrix and invert it
# twiss_outputs_files = glob.glob('twiss_outputs/twiss*knl1*.csv')
...
# Uncomment this for the skew correctors
twiss_outputs_files = glob.glob('twiss_outputs/twiss*ksl1*.csv')
RM, corr_knob_names, dk_list = build_response_matrix(twiss_outputs_files, OBSERVATION_POINTS, OBSERVABLES, vector0, print_progress=True)
